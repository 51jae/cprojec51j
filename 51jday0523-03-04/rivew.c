...
int main0()
{
	...
	//반복문 while
	//while은 플래그 i가 필요,중복코드를 관리
	int i = 0;
	while (i < 3)
	{
		printf("반복횟수 %d", i + 1);//i보다 1 더 큰 숫자 출력
		i++; //변수 i 에 들어있는 값을 1증가 시킴 i=i+1과 같다
	}
	for (int i = 0; i < 3; i++)
	{
		printf("\n반복횟수 %d회", i + 1); //반복하고자하는 코드만 남음
	}

	//기타제어문 continue , break
	//break: 반복문 종료 (swich종료)
	//continue : 반복문 처음으로()로 돌아감 (1회성 취소)
	for (int i = 0; i < 3; i++)
	{
		if (i == 2)
		{
